...
selected_gene <- c("IL7R", "OASL", "SKAP1", "DHX58", "CXCL11", "IFIT3", "DDX58", "GBP4", "ZBTB32",
                   "VSTM1", "OAS2", "CCL5", "IFIT1", "RTP4", "VAV1", "TNFSF10", "EBLN1", "IFIT1B",
                   "RUFY4", "MRC1", "BTK", "CCL3L3", "LILRA2", "AIM2", "CTSE")

ui <- fluidPage(
  titlePanel("Volcano and MA Plot Generator"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  volcanoPlot <- reactive({
    ...
    # Combine user-defined additional genes with the pre-defined selected_gene vector
    ...
    # Add a 'text' aesthetic for tooltips if interactive
  ...
  maPlot <- reactive({
    ...
    # Combine user-defined additional genes with the predefined selected_gene vector
    extra_genes_vec <- unlist(strsplit(input$extra_genes, "\\s+"))
    extra_genes_vec <- extra_genes_vec[extra_genes_vec != ""]
    label_genes <- unique(c(selected_gene, extra_genes_vec))
